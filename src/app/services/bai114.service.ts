...
import { Bai114, Book } from '../interfaces/bai114';

@Injectable({
  providedIn: 'root'
})
export class Bai114Service {

  constructor(private _http: HttpClient) { }


  getBooks(): Observable<Array<Bai114>> {
    ...
    return this._http.get<any>("/books", requestOptions).pipe(
      map(res => JSON.parse(res) as Array<Bai114>),
      retry(3),
  ...
  getBook(bookId: string): Observable<Bai114> {
    const headers = new HttpHeaders().set("Content-Type", "text/plain;charset=utf-8")
    const requestOptions: Object = {
      headers: headers,
      responseType: "text"
    ...
    return this._http.get<any>("/books/" + bookId, requestOptions).pipe(
      map(res => JSON.parse(res) as Bai114),
      retry(3),
      catchError(this.handleError))
  }
